...
class CIFAR10(dataset.Dataset):
    ...
    def _download_cache_data(self):
        """
        使用本地已有的 CIFAR-10 数据生成 data.npz，完全跳过下载。
        确保 base_path 下已有 cifar-10-batches-py/ 文件夹。
        """
        print("Using local CIFAR-10 archive")

        batch_dir = os.path.join(self.base_path, "cifar-10-batches-py")
        if not os.path.exists(batch_dir):
            raise FileNotFoundError(f"{batch_dir} 不存在，请先上传 CIFAR-10 数据集")
    ...
    def _load_data(self) -> Dict[str, DataTuple]:
        ...
        return {
            'train': DataTuple((data['x_train'], data['y_train'])), 
